...
if __name__ == '__main__':
    ...
    adj_matrix = sparse.load_npz('output/graph.npz')
    ...
    model.add(GraphConvoluation(adj_matrix=adj_matrix, units=256, activation=tf.nn.relu, kernel_regularizer=1e-5, bias_regularizer=1e-5, input_shape=(24,)))
    model.add(GraphConvoluation(adj_matrix=adj_matrix, units=256, activation=tf.nn.relu, kernel_regularizer=1e-5, bias_regularizer=1e-5))
    model.add(Dropout(0.8))
    model.add(GraphConvoluation(adj_matrix=adj_matrix, units=node_labels.shape[1]))

    optimizer = Optimizer(lr=1e-2, tf_optimizer=tf.train.RMSPropOptimizer, decay=0.995)
    ...
    model.fit(node_features, node_labels, epochs=10000, save_path=model_path, k=30)
